...
@Component({
  selector: 'app-project-weightage',
  templateUrl: './project-weightage.component.html',
  styleUrls: ['./project-weightage.component.css']
})
export class ProjectWeightageComponent implements OnInit {
    ...
    if (total === 100 && !this.checkRmExistErrorInfo) {
      ...
      this.appraisalService.updateProjectWeight(this.projectWeightageInfo).subscribe(
        (response) => {
          ...
          this.activeModal.close({projectList:this.projectWeightageForm.value['projectWeightageList'],type:"save"});
          this.appraisalService.empOverallRating = response['empOverallRating'].toFixed(2);
          this.appraisalService.empSelfRating = response['empSelfRating'].toFixed(2);
        },
        (error) => {
