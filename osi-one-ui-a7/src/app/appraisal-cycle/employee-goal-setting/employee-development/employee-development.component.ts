...
export class EmployeeDevelopmentComponent implements OnInit {
  ...
  { name: "No", value: 0 }]
  empsdevelopment: FormGroup;
  empsimprovement: FormGroup;
  ...
  responseObj = {};
  commentsInfo;
  hasDevDuplicateKpas = false;
  hasImpDuplicateKpas = false;
  minDate;
  checkYearInfo = false;
  ...
  ngOnInit() {
    ...
    this.appraisalService.breadcrumbText = 'Career';
  ...
  getEmpsEmpDetails() {
    ...
    this.appraisalService.getEmployeeDevelopmentInfo(employeeId, epmsHdrId, 'CAREER').subscribe(response => {
      ...
      // this.appraisalService.getStatusDescription(this.osiEpmsEmpDetails['status'])
  ...
  populateIndividualData(response) {
    const commonIDsDev = [];
    const commonIDsImprove = [];
    const objDev = response['osiEpmsEmpDetails'][0].osiEpmsEmpKraDetails[0].osiEpmsEmpKpaDetails;
    const objImpr = response['osiEpmsEmpDetails'][0].osiEpmsEmpKraDetails[1].osiEpmsEmpKpaDetails;
    const formArrayValuesdev = this.empsdevelopment.value.osiEpmsEmpKpaDetails;
    const formArrayValuesImprove = this.empsimprovement.value.osiEpmsEmpImpKpaDetails;
    objDev.forEach(element => {
      commonIDsDev.push(element.kpa);
    });
    objImpr.forEach(element => {
      commonIDsImprove.push(element.kpa);
    });
    formArrayValuesdev.forEach(element => {
      commonIDsDev.push(element.kpa);
    });
    formArrayValuesImprove.forEach(element => {
      commonIDsImprove.push(element.kpa);
    });
    const resultDev = commonIDsDev.filter((e, i, a) => a.indexOf(e) !== i);
    const resultImp = commonIDsImprove.filter((e, i, a) => a.indexOf(e) !== i);
    if (this.passedValue === 'dev') {
      if (resultDev.length < 1) {
        this.developmentAreasList = response['osiEpmsEmpDetails'][0].osiEpmsEmpKraDetails ? response['osiEpmsEmpDetails'][0].osiEpmsEmpKraDetails[0] : [];
        this.populateOsiEpmsEmpKpaDetails_Development(this.developmentAreasList);
    ...
    if (this.passedValue === 'improve') {
      if (resultImp.length < 1) {
        this.imporvementAreasList = response['osiEpmsEmpDetails'][0].osiEpmsEmpKraDetails ? response['osiEpmsEmpDetails'][0].osiEpmsEmpKraDetails[1] : [];
        this.populateOsiEpmsEmpKpaDetails_Improvement(this.imporvementAreasList);
        this.toastr.success('Goals are added to the Employee');
      } else {
  ...
  onClickmyGoals() {
    ...
    (<MyGoalsEmpdevelopmentComponent>modalReference.componentInstance).isFrom = 'CAREER';
    ...
    // (<MyGoalsEmpdevelopmentComponent>modalReference.componentInstance).myGoalsData = response;
  ...
  preformanceAreas() {
    this.empsdevelopment = this.formBuilder.group({
      osiEpmsEmpKpaDetails: this.formBuilder.array([])
    });
  }

  performanceAreas_Imp() {
    this.empsimprovement = this.formBuilder.group({
      osiEpmsEmpImpKpaDetails: this.formBuilder.array([])
    });
  }

  get empKparatingInfo(): FormArray {
    return this.empsdevelopment.get('osiEpmsEmpKpaDetails') as FormArray;
  }

  get empKparatingInfoforImp(): FormArray {
    return this.empsimprovement.get('osiEpmsEmpImpKpaDetails') as FormArray;
  }
  ...
  saveempDevelopementAreas(status): any {
    ...
    this.hasDevDuplicateKpas = false;
    this.hasImpDuplicateKpas = false;
    ...
    if (this.responseObj['osiEpmsEmpDetails'][0]['osiEpmsEmpKraDetails'][0] &&
      ...
      const empDevKpaDetails = this.responseObj['osiEpmsEmpDetails'][0]['osiEpmsEmpKraDetails'][0]['osiEpmsEmpKpaDetails'];
      const empDevKpaNames = empDevKpaDetails.map(kpaInfo => kpaInfo['kpa']);
      const getDevDuplicateKpaNames = this.appraisalService.checkKpaNamesDuplicates(empDevKpaNames);
      if (getDevDuplicateKpaNames && getDevDuplicateKpaNames.length) {
        this.hasDevDuplicateKpas = true;
      }
    }
    ...
    if (this.responseObj['osiEpmsEmpDetails'][0]['osiEpmsEmpKraDetails'][1] &&
      ...
      const empImpKpaDetails = this.responseObj['osiEpmsEmpDetails'][0]['osiEpmsEmpKraDetails'][1]['osiEpmsEmpKpaDetails'];
      const empImpKpaNames = empImpKpaDetails.map(kpaInfo => kpaInfo['kpa']);
      const getImpDuplicateKpaNames = this.appraisalService.checkKpaNamesDuplicates(empImpKpaNames);
      if (getImpDuplicateKpaNames && getImpDuplicateKpaNames.length) {
        this.hasImpDuplicateKpas = true;
      }
    }

    if (!this.checkKpasLength && !this.displayCommentsErrorMsg && !this.displayRatingsErrorMsg && !this.hasDevDuplicateKpas && !this.hasImpDuplicateKpas) {
      if (status === "DECLINED") {
        const modalReference = this.modalService.open(DeclineCommentsComponent);
        this.responseObj['categoryType'] = "Career";
      ...
      if (this.hasDevDuplicateKpas) {
        this.toastr.error("Duplicate Performance Area found in Development");
      }
      if (this.hasImpDuplicateKpas) {
        this.toastr.error("Duplicate Performance Area found in Improvement");
      }
  ...
  updateDevKpaInfo(index): void {
    ...
    if (this.loginId == this.employeeDetails.employeeId && this.empStatus !== this.STATUS_CODE_INFO['CLOSED']) {
