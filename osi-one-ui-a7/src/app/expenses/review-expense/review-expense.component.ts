...
@Component({
  ...
  templateUrl: './review-expense.component.html',
  styleUrls: ['./review-expense.component.css']
})
export class ReviewExpenseComponent implements OnInit {
  ...
  onRowChecked(event) {
    if (event.target.checked) {
      ...
      this.isEnableWaitingOn = true;
    } else {
      this.selectedRowId = 0;
      this.isEnableWaitingOn = false;
    }
  }
 ...
 getCurr(val){
  ...
  getExpenseForApproval () {
    ...
    .subscribe(waitingOn => {
      ...
      //console.log(" Waiting On : " + JSON.stringify(this.waitingOn));
    },
      error => this.errorMessage = <any>error);
  }
