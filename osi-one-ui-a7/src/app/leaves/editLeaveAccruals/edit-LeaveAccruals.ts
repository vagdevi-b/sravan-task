...
@Component({
  selector: "edit-LeaveAccruals",
  templateUrl: "./edit-LeaveAccruals.html",
  styleUrls: ["./edit-LeaveAccruals.css"]
})
export class EditLeaveAccuralsComponent implements OnInit {
  ruleId: number;
  accrualDetails: any[] = [];
  // allLocation: any[] = [];
  leaveTypesList: any[] = [];
  ...
  getLeaveAccrualDetails(id) {
    this._accuralRuleService.getAccrualsByruleId(id).subscribe(resp => {
      ...
      this.leaveTypesList.forEach(x=>{
        if(x.leaveTypeId === this.user.leaveTypeId){
            if(x.leaveTypeName.toLowerCase() === "PAID TIME OFF (PTO)".toLowerCase() ){
              this.carryFDays = 'PAID';
              this.isPto=true;
            }
        }
      })
    ...
    ()=>{},
    ()=>{
      this._accuralRuleService.getLocationByOrgId(this.user.orgId).subscribe(response => {
        this.allLocationByOrgId = response;
      })
    }
    );
  ...
  updateLeaveAccurals(user) {
    ...
    if(this.carryFDays && this.carryFDays=='NON'){
      this.user.carryForwardDays = 0;
    }
    if(user.ruleId != null){
    this._accuralRuleService
      .updateLeaveAccurals(user)
  ...
  converter1(key,event){
    ...
    // if(this.selectedleaveTypeName=="PAID TIME OFF (PTO)"){
